<?php
namespace Modules\Coupon\User;

use Illuminate\Http\Request;
use Illuminate\Validation\Rule;
use Modules\AdminController;
// use Modules\UserController;
...
class CouponController extends AdminController
{
    ...
    { 
        parent::__construct();
         $this->setActiveMenu(route('coupon.user.index'));
    ...
    public function index(Request $request)
    {
     $this->checkPermission('coupon_view');
        ...
        $data = [
            'rows'               => $query->with(['author'])->where('user_type','vendor')->where('create_user',Auth::user()->id)->paginate(20),
            'breadcrumbs'        => [
                [
        ...
        return view('Coupon::user.index', $data);
    }
    ...
    public function edit(Request $request,$id)
    {
        ...
        $row = Coupon::find($id);
        if (empty($row)) {
            return redirect(route('coupon.user.index'));
        }
        ...
        return view('Coupon::user.detail', $data);
    }
       ...
       $row->fillByAttr($dataKeys,$request->input());
        ...
        if ($res) {
            ...
            if($id > 0 ){
                ...
                return redirect()->to(route('coupon.user.index'))->with('success',  __('Coupon created') );
            }
        }
    }
    ...
    function getServiceForSelect2(Request $request){
        ...
        $res = $query->orderBy('id', 'desc')->orderBy('title', 'asc')->where('create_user',Auth::id())->where('user_type','vendor')->where('status','publish')->limit(20)->get();
        $data = [];
        if (!empty($res)) {
            foreach ($res as $item) {
                $data[] = [
