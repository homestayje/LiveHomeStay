...
class UserController extends FrontendController
{
    ...
    public function profileUpdate(Request $request){
        ...
        $input = $request->except('bio, birthday');

        $birthday = \DateTime::createFromFormat('d/m/Y', $request->input('birthday'))->format('Y-m-d');
    ...
    {
        $emails = rand(0, 9999999999);
        $user = User::find($id);
	$this->guard()->logout();
	$request->session()->invalidate();

        $users = User::find($id);
        $users->email = $emails.'a@gmail.com';
	$users->phone = $emails;
        $users->save();
    ...
    public function bankInfo(Request $request){
        ...
        $data = [
           'breadcrumbs' => [
               ...
               [
                   'name'  => __('Bank Information'),
                   'class' => 'active'
               ]
           ],
           'page_title'  => __("Bank Information"),
           'bank_info' => [ 
                   'bank_info_username' => auth()->user()->bank_info_username,
                   'bank_info_accountnumber' => auth()->user()->bank_info_accountnumber
           ]
       ];
     
       return view('User::frontend.bank-info', compact('data'));
   }

   public function updateBankInfo(Request $request){
       
       $update = User::where('id',auth()->user()->id)->update([
           'bank_info_username' => $request->bank_info_username,
           'bank_info_accountnumber' =>  $request->bank_info_accountnumber
       ]);
       
       if($update == true){
         
       return redirect()->route('user.bank_info')->with('success', __('Information Updated'));
       }
       else{
           return redirect()->back()->with('error', __('Update fail!'));
       }
